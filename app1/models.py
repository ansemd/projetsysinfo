...
class Chauffeur(models.Model):
    ...
    CATEGORIE_PERMIS_CHOICES = [
        ...
        ('EC', 'Catégorie EC - Poids lourds avec remorque'),
    ]
    categorie_permis = models.CharField(max_length=5, choices=CATEGORIE_PERMIS_CHOICES)
...
class Vehicule(models.Model):
    ...
    kilometrage = models.PositiveIntegerField(
        default=0,
        help_text="Compteur kilométrique total du véhicule"
    )
...
class Destination(models.Model):
    ...
    zone_logistique = models.CharField(
        max_length=10, 
        choices=ZONE_LOGISTIQUE_CHOICES, 
        default='CENTRE',
        help_text="Utilisé pour regrouper les expéditions dans une même tournée"
    )

    distance_estimee = models.IntegerField(
        help_text="Distance en km depuis le dépôt principal"
    )
    ...
    tarif_base = models.DecimalField(
        ...
        default=0,
        editable=False, 
        help_text="Tarif de base en DA (calculé : distance × 20)"
    )
    ...
    delai_livraison_estime = models.IntegerField(
        default=1,
        editable=False,  
        help_text="Délai estimé en jours (calculé : 1 jour par 30 km)"
    )
    ...
    def save(self, *args, **kwargs):
        self.tarif_base = Decimal(str(self.distance_estimee)) * Decimal('20.00')
        self.delai_livraison_estime = math.ceil(self.distance_estimee / 30)
        if self.delai_livraison_estime == 0:
            self.delai_livraison_estime = 1
...
class TypeService(models.Model):
    ...
    multiplicateur_prix = models.DecimalField(
        max_digits=5, 
        decimal_places=2,
        editable=False,
        default=1.00,
        help_text="Multiplicateur du prix (1.00 = normal, 2.00 = x2 plus cher)"
    )
    multiplicateur_delai = models.DecimalField(
        max_digits=5, 
        decimal_places=2,
        editable=False,
        default=1.00,
        help_text="Multiplicateur du délai (0.50 = 2x plus rapide, 1.00 = normal)"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.type_service == 'STANDARD':
            self.multiplicateur_prix = 1.00
            self.multiplicateur_delai = 1.00
        elif self.type_service == 'EXPRESS':
            self.multiplicateur_prix = 2.00
            self.multiplicateur_delai = 0.50
        
        super().save(*args, **kwargs)
    
    def __str__(self):
        return f"{self.get_type_service_display()}"

class Tarification(models.Model):
    type_service = models.ForeignKey(TypeService, on_delete=models.CASCADE)
    destination = models.ForeignKey(Destination, on_delete=models.CASCADE)
    
    tarif_poids = models.DecimalField(max_digits=10, decimal_places=2, default=10.00)
    tarif_volume = models.DecimalField(max_digits=10, decimal_places=2, default=20.00)
    ...
    @classmethod
    def obtenir_calculs(cls, destination, type_service, poids, volume):
        """
        C'est la fonction magique. Elle cherche une tarification, 
        si elle ne la trouve pas, elle utilise les valeurs par défaut.
        """
        from decimal import Decimal
        
        # 1. On cherche si une règle spécifique existe
        instance = cls.objects.filter(destination=destination, type_service=type_service).first()
        
        if instance:
            # Si elle existe, on utilise ses tarifs personnalisés
            t_poids = instance.tarif_poids
            t_volume = instance.tarif_volume
        else:
            # Sinon, on utilise les tarifs standards par défaut
            t_poids = Decimal('10.00')
            t_volume = Decimal('20.00')
            
        # 2. Calcul du montant
        p_dec = Decimal(str(poids))
        v_dec = Decimal(str(volume))
        
        montant_base = destination.tarif_base + (p_dec * t_poids) + (v_dec * t_volume)
        montant_final = montant_base * type_service.multiplicateur_prix
        
        # 3. Calcul du délai
        delai_final = Decimal(str(destination.delai_livraison_estime)) * type_service.multiplicateur_delai
        
        return montant_final, delai_final
